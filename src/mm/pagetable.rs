...
impl PageTable {
    pub fn vm_print(&self) {
        ...
        self.print_level(self.root(), 2);
    }

    fn print_level(&self, pa: PhysAddr, level: usize) {
        let ptr = pa.as_usize() as *const PageTableEntry;
        for i in 0..512 {
            let pte = unsafe { *ptr.add(i) };
            if !pte.is_valid() {
                continue;
            }
            match level {
            ...
            println!("{}: pte {:#x} pa {:#x}", i, pte.data, child_pa.as_usize());
            if level > 0 {
                self.print_level(child_pa, level - 1);
            }
        }
    }
    ...
    fn root(&self) -> PhysAddr {
        ...
        let ppn = satp & !(SATP_SV39);
        unsafe { PhysAddr::from_raw(ppn << PGSHIFT) }
    }
    ...
    pub fn map_pages(
        ...
        while va != last {
            match self.walk_alloc(va) {
                Some(pte) => {
                    if pte.is_valid() {
                        println!("va: {:#x}, pa: {:#x}, pte: {:#x}", va.as_usize(), pa.as_usize(), pte.data);
                        panic!("remap");
                    }
                ...
                None => {
                    return Err("PageTable.map_pages: not enough memory for new page table")
                }
            }
        }
        Ok(())
    ...
    pub fn walk_addr_mut(&mut self, va: VirtAddr) -> Result<PhysAddr, &'static str> {
    ...
    pub fn alloc_proc_pagetable(trapframe: usize) -> Option<Box<Self>> {
        ...
        pagetable.map_pages(VirtAddr::from(TRAMPOLINE), PGSIZE, PhysAddr::try_from(trampoline as usize).unwrap(), PteFlag::R | PteFlag::X).ok()?;
        pagetable.map_pages(VirtAddr::from(TRAPFRAME), PGSIZE, PhysAddr::try_from(trapframe).unwrap(), PteFlag::R | PteFlag::W).ok()?;
        Some(pagetable)
    }
    ...
    pub fn uvm_init(&mut self, code: &[u8]) {
        if code.len() >= PGSIZE { panic!("initcode more than a page"); }
        let mem = unsafe { RawSinglePage::new_zeroed() as *mut u8 };
        self.map_pages(VirtAddr::from(USERTEXT), PGSIZE, PhysAddr::try_from(mem as usize).unwrap(), PteFlag::R | PteFlag::W | PteFlag::X | PteFlag::U).expect("map_page error");
    ...
    pub fn uvm_alloc(&mut self, old_size: usize, new_size: usize) -> Result<usize, ()> {
        if new_size <= old_size { return Ok(old_size) }
        ...
        for cur_size in (old_size..new_size).step_by(PGSIZE) {
            match unsafe { RawSinglePage::try_new_zeroed() } {
                ...
                Ok(mem) => {
                    match self.map_pages(unsafe { VirtAddr::from_raw(cur_size) }, PGSIZE, unsafe { PhysAddr::from_raw(mem as usize) }, PteFlag::R | PteFlag::W | PteFlag::X | PteFlag::U) {
                        Err(_s) => {
    ...
    pub fn uvm_unmap(&mut self, va: usize, count: usize, freeing: bool) {
        if va % PGSIZE != 0 { panic!("va not page aligned"); }
        for ca in (va..(va+PGSIZE*count)).step_by(PGSIZE) {
            let pte = self.walk_mut(unsafe {VirtAddr::from_raw(ca)}).expect("unable to find va available");
            if !pte.is_valid() { panic!("this pte is not valid"); }
            if !pte.is_leaf() { panic!("this pte is not a leaf"); }
            if freeing {
    ...
    pub fn uvm_clear(&mut self, va: usize) {
        let pte = self.walk_mut(VirtAddr::try_from(va).unwrap()).expect("cannot find available pte");
        pte.clear_user();
    }
    ...
    pub fn uvm_copy(&mut self, child_pgt: &mut Self, size: usize) -> Result<(), ()> {
        for i in (0..size).step_by(PGSIZE) {
            ...
            if let Ok(mem) = mem {
                ...
                if child_pgt.map_pages(va, PGSIZE, unsafe { PhysAddr::from_raw(mem as usize) }, perm).is_ok() {
                    continue
                }
                unsafe { RawSinglePage::from_raw_and_drop(mem); }
            }
        ...
        while i < dst.len() {
            ...
            while count > 0 {
                unsafe {
                    ...
                    i += 1; count -= 1; pa_ptr = pa_ptr.add(1);
                }
            }
            base.add_page(); va = base;
        }
        Err("copy_in_str: dst not enough space")
    }

    pub fn copy_out(&mut self, mut src: *const u8, mut dst: usize, mut count: usize) -> Result<(), ()> {
        ...
        loop {
            ...
            let dst_ptr = unsafe { pa.as_mut_ptr().offset(off as isize) };
    ...
    pub fn copy_in(&self, mut src: usize, dst: *mut u8, mut count: usize) -> Result<(), ()> {
        ...
        loop {
            ...
            let off_from_end = PGSIZE - off;
            let src_ptr = unsafe { pa.as_ptr().offset(off as isize) };
            if off_from_end > count {
            ...
            count -= off_from_end;
            src += off_from_end;
            unsafe { dst.offset(off_from_end as isize) };
            va.add_page();
        }
    }
}
